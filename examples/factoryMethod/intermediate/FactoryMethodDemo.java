package factoryMethod.intermediate;
...
class ButtonCreatorImpl implements ButtonCreator{
	 public JButton createButton(){
...
class LabelCreatorImpl implements LabelCreator{
	public JLabel createLabel(){
...
class GUIComponentCreator {
    ...
    public final void showFrame(String title, int choice) {
		...
		if(choice == 0)
			panel.add(((ButtonCreator)new ButtonCreatorImpl()).createButton());
		else
			panel.add(((LabelCreator) new LabelCreatorImpl()).createLabel());
